...
const demoTransactions: Transaction[] = [{ swifttip_reference: "ST-26-8F3K9D", completed_at: new Date().toISOString(), worker_display_name: "Thando", venue_name: "Riverside Service Station", customer_total_cents: 5250, gross_gratuity_cents: 5000, worker_net_cents: 4750, swifttip_gross_revenue_cents: 500, provider_cost_cents: 290, contribution_cents: 210, settlement_state: "pending" }];
...
export default async function AdminPage() {
  ...
  return (
    <main className="dashboard-shell">
      ...
      <section className="dashboard-title"><span className="eyebrow">Operations</span><h1>Attention first.</h1><p className="lead">Financial exceptions and required actions take priority over vanity metrics.</p></section>
      <div className="metric-grid" style={{ marginTop: 22 }}>
        <article className="metric-card"><span>Settlement exceptions</span><strong>{Number(dashboard.settlement_exceptions)}</strong><small>Needs review</small></article>
        <article className="metric-card"><span>Reconciliation exceptions</span><strong>{Number(dashboard.reconciliation_exceptions)}</strong><small>Needs review</small></article>
        <article className="metric-card"><span>Pending verification</span><strong>{Number(dashboard.pending_verifications)}</strong></article>
        <article className="metric-card"><span>Open disputes</span><strong>{Number(dashboard.open_disputes)}</strong></article>
      </div>

      <div className="admin-grid">
        <section className="dashboard-section"><h2>7-day commercial pulse</h2><div className="money-breakdown"><div className="money-row"><span>Successful tips</span><strong>{Number(dashboard.successful_tips_7d)}</strong></div><div className="money-row"><span>Gross gratuity value</span><strong>{formatZar(Number(dashboard.gross_gratuity_7d_cents))}</strong></div><div className="money-row"><span>SwiftTip gross fee revenue</span><strong>{formatZar(Number(dashboard.swifttip_gross_revenue_7d_cents))}</strong></div><div className="money-row"><span>Provider direct cost</span><strong>− {formatZar(Number(dashboard.provider_cost_7d_cents))}</strong></div><div className="money-row total"><span>Transaction contribution</span><strong>{formatZar(Number(dashboard.contribution_7d_cents))}</strong></div></div><p className="fee-note">Contribution is not profit. It is SwiftTip gross transaction revenue less recorded direct provider costs.</p></section>
        <section className="dashboard-section"><h2>Queues</h2><Link className="queue-row" href="/admin/verifications"><div><strong>Worker verification</strong><div className="meta">{Number(dashboard.pending_verifications)} requiring attention</div></div><span>→</span></Link>{financialRole && <><Link className="queue-row" href="/admin/settlements"><div><strong>Settlement exceptions</strong><div className="meta">{Number(dashboard.settlement_exceptions)} requiring attention</div></div><span>→</span></Link><Link className="queue-row" href="/admin/transactions"><div><strong>Transactions</strong><div className="meta">Financial traceability</div></div><span>→</span></Link></>}</section>
      </div>
