...
export default async function AdminPage(){
  ...
  return <main className="dashboard-shell admin-shell-polished">
    <div className="admin-hero"><section className="dashboard-title"><span className="eyebrow">Operations overview</span><h1>{totalAttention ? `${totalAttention} items need attention.` : "No urgent exceptions."}</h1><p className="lead">Resolve exceptions and verification work first. Commercial metrics stay visible without competing with operational risk.</p></section><span className={access.mode==="demo"?"status-chip warning":"status-chip success"}>{access.mode==="demo"?"Preview data":access.role.replaceAll("_"," ")}</span></div>
    ...
    <div className="admin-grid">
      ...
      <section className="dashboard-section admin-quick-actions"><div className="section-heading"><div><span className="eyebrow">Priority queues</span><h2>Go where the work is</h2></div></div><div className="control-grid">
