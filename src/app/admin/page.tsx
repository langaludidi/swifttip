...
export default async function AdminPage(){
  ...
  return <main className="dashboard-shell">
    ...
    <div className="admin-grid"><section className="dashboard-section"><h2>7-day commercial pulse</h2><div className="money-breakdown"><div className="money-row"><span>Successful tips</span><strong>{Number(dashboard.successful_tips_7d)}</strong></div><div className="money-row"><span>Gross gratuity value</span><strong>{formatZar(Number(dashboard.gross_gratuity_7d_cents))}</strong></div><div className="money-row"><span>SwiftTip gross fee revenue</span><strong>{formatZar(Number(dashboard.swifttip_gross_revenue_7d_cents))}</strong></div><div className="money-row"><span>Provider direct cost</span><strong>− {formatZar(Number(dashboard.provider_cost_7d_cents))}</strong></div><div className="money-row total"><span>Transaction contribution</span><strong>{formatZar(Number(dashboard.contribution_7d_cents))}</strong></div></div><p className="fee-note">Contribution is not profit. It is gross transaction revenue less recorded direct provider costs.</p></section>
      <section className="dashboard-section"><h2>Control surfaces</h2>
        {verificationRole&&<Link className="queue-row" href="/admin/verifications"><div><strong>Worker verification</strong><div className="meta">{Number(dashboard.pending_verifications)} requiring attention</div></div><span>→</span></Link>}
        {operationsRole&&<><Link className="queue-row" href="/admin/venues"><div><strong>Venue register</strong><div className="meta">Create and approve participating locations</div></div><span>→</span></Link><Link className="queue-row" href="/admin/legal"><div><strong>Legal document register</strong><div className="meta">Draft, review and approve — publication remains separate</div></div><span>→</span></Link><Link className="queue-row" href="/admin/pilot/setup"><div><strong>Pilot setup</strong><div className="meta">Draft cohorts and Venue assignment only</div></div><span>→</span></Link></>}
        {financialRole&&<><Link className="queue-row" href="/admin/settlements"><div><strong>Settlement exceptions</strong><div className="meta">{Number(dashboard.settlement_exceptions)} requiring attention</div></div><span>→</span></Link><Link className="queue-row" href="/admin/transactions"><div><strong>Transactions</strong><div className="meta">Financial traceability</div></div><span>→</span></Link><Link className="queue-row" href="/admin/refunds"><div><strong>Refunds</strong><div className="meta">{Number(dashboard.refund_requests)} records requiring/awaiting processing</div></div><span>→</span></Link><Link className="queue-row" href="/admin/disputes"><div><strong>Disputes</strong><div className="meta">{Number(dashboard.open_disputes)} open or evidence-stage cases</div></div><span>→</span></Link><Link className="queue-row" href="/admin/readiness"><div><strong>Commercial readiness</strong><div className="meta">Pricing, terms and activation gates</div></div><span>→</span></Link><Link className="queue-row" href="/admin/pilot"><div><strong>Pilot scorecard</strong><div className="meta">Evidence-based commercial and operational metrics</div></div><span>→</span></Link></>}
        {supportRole&&<Link className="queue-row" href="/admin/support"><div><strong>Support cases</strong><div className="meta">Controlled operational triage</div></div><span>→</span></Link>}{auditRole&&<Link className="queue-row" href="/admin/audit"><div><strong>Audit trail</strong><div className="meta">Restricted security visibility</div></div><span>→</span></Link>}
      </section></div>
