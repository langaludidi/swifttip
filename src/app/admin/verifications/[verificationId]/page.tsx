...
type VerificationDetail = {
  ...
  identity_number_last4: string | null;
...
export default async function VerificationReviewPage({ params, searchParams }: { params: Promise<{ verificationId: string }>; searchParams: Promise<{ error?: string; decided?: string }> }) {
  ...
  if (access.mode === "demo") {
    rows = [{ verification_id: "demo", worker_id: "demo-worker", legal_first_name: "Nomsa", legal_last_name: "Mokoena", display_name: "Nomsa", verification_type: "identity", verification_status: "submitted", submitted_at: new Date().toISOString(), reviewed_at: null, decision_reason: null, venue_name: "Riverside Service Station", identity_document_type: "sa_smart_id", identity_number_last4: "0000", identity_consented_at: new Date().toISOString(), selfie_capture_method: "browser_camera", selfie_captured_at: new Date().toISOString(), document_id: "demo-document", storage_path: null, document_type: "identity_document", mime_type: "application/pdf", file_size_bytes: 182000, uploaded_at: new Date().toISOString(), storage_object_available: true }];
  } else {
  ...
  return (
    <main className="dashboard-shell">
      ...
      <div className="admin-grid">
        <section className="dashboard-section"><h2>Worker identity</h2><div className="money-breakdown"><div className="money-row"><span>Legal name</span><strong>{verification.legal_first_name} {verification.legal_last_name}</strong></div><div className="money-row"><span>Public name</span><strong>{verification.display_name}</strong></div><div className="money-row"><span>Document type</span><strong>{verification.identity_document_type?.replaceAll("_", " ") ?? "Not recorded"}</strong></div><div className="money-row"><span>Identity number</span><strong>{verification.identity_number_last4 ? `Protected · ends ${verification.identity_number_last4}` : "Not recorded"}</strong></div><div className="money-row"><span>Identity consent</span><strong>{when(verification.identity_consented_at)}</strong></div><div className="money-row"><span>Selfie capture</span><strong>{verification.selfie_capture_method?.replaceAll("_", " ") ?? "Not recorded"}</strong></div><div className="money-row"><span>Venue context</span><strong>{verification.venue_name ?? "None"}</strong></div><div className="money-row"><span>Submitted</span><strong>{when(verification.submitted_at)}</strong></div><div className="money-row total"><span>Status</span><strong>{verification.verification_status.replaceAll("_", " ")}</strong></div></div>{verification.decision_reason && <p className="fee-note">Previous reviewer note: {verification.decision_reason}</p>}</section>
      ...
      {canDecide && <section className="dashboard-section"><h2>Decision</h2><div className="admin-grid">{canApprove ? <form action={decideVerification} className="stack-actions verification-checklist"><input type="hidden" name="verificationId" value={verification.verification_id}/><input type="hidden" name="decision" value="approve"/><p className="lead">Open every file and complete all three confirmations. Phase 1 approval is a manual identity decision.</p><label className="consent-check"><input type="checkbox" name="documentMatches" value="yes" required/><span>The legal name and identity-number ending match the submitted identity document.</span></label><label className="consent-check"><input type="checkbox" name="selfieMatches" value="yes" required/><span>The live selfie reasonably resembles the photograph on the identity document.</span></label><label className="consent-check"><input type="checkbox" name="duplicateClear" value="yes" required/><span>No duplicate-account warning or unexplained evidence inconsistency is present.</span></label><button className="button button-primary" type="submit">Approve identity verification</button></form> : <div className="stack-actions"><p className="lead">Approval remains unavailable until the identity document and live selfie can both be reviewed.</p><button className="button button-primary" type="button" disabled>Approve identity verification</button></div>}<div className="stack-actions"><form action={decideVerification} className="stack-actions"><input type="hidden" name="verificationId" value={verification.verification_id}/><input type="hidden" name="decision" value="request_more_information"/><label className="field-label" htmlFor="more-info-reason">Request more information</label><textarea id="more-info-reason" name="reason" rows={3} required placeholder="What additional evidence is required?"/><button className="button" type="submit">Request more information</button></form><form action={decideVerification} className="stack-actions"><input type="hidden" name="verificationId" value={verification.verification_id}/><input type="hidden" name="decision" value="reject"/><label className="field-label" htmlFor="reject-reason">Reject verification</label><textarea id="reject-reason" name="reason" rows={3} required placeholder="Reason for rejection"/><button className="button" type="submit">Reject verification</button></form></div></div></section>}
    </main>
  );
}
