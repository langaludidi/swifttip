...
type AuditEvent = Database["public"]["Functions"]["admin_get_audit_events"]["Returns"][number];
...
export default async function AdminAuditPage({ searchParams }:{searchParams:Promise<{entity?:string}>}){
  ...
  let events:AuditEvent[]=access.mode==="demo"?[{audit_id:"demo",actor_type:"admin",actor_user_id:"",actor_role:"operations_admin",action:"support_case_status_changed",entity_type:"support_case",entity_id:"",reason:"Example audit event",created_at:new Date().toISOString()}]:[];
  if(access.mode==="live"){
    const supabase=await createSupabaseServerClient();
    const pEntityType=entity?.trim()||undefined;
    const {data}=await supabase.rpc("admin_get_audit_events",{p_limit:100,p_entity_type:pEntityType});
    events=data??[];
  }
