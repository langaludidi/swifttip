...
export default async function WorkerProfilePage() {
  ...
  return <main className="mobile-app-shell"><div className="app-page worker-home-page">
    ...
    <section className="dashboard-section"><div className="section-heading"><div><span className="eyebrow">Settlement</span><h2>Your destination</h2></div><span className={chip(settlementDone)}>{label(context.settlement_readiness)}</span></div><div className="list-row"><div><strong>Provider settlement profile</strong><div className="meta">{context.masked_destination??"No verified destination yet"}</div></div></div><div className="privacy-inline"><span>✓</span><p>SwiftTip shows only masked provider information. Funds move through the approved provider; SwiftTip does not expose an internal stored-value account or manual payout instruction.</p></div></section>
