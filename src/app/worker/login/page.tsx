...
export default async function WorkerLoginPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return <main className="flow-shell customer-flow-page"><div className="flow-page"><header className="simple-header"><Link className="back-link" href="/">←</Link><div className="brand-lockup"><AppMark size={34}/><div><strong>SwiftTip</strong><span className="brand-subline">Worker</span></div></div><span style={{width:42}}/></header>
    ...
    <form action={requestWorkerOtp} className="code-entry-card"><label className="field-label" htmlFor="phone">Mobile number</label><div className="code-input-shell"><span>+27</span><input id="phone" name="phone" inputMode="tel" placeholder="82 555 0199" autoComplete="tel" required/></div><button className="button button-primary button-large" type="submit">Send one-time code</button></form>
    <div className="privacy-inline"><span>✓</span><p>There is no universal SwiftTip login code. Access is confirmed through the configured authentication service.</p></div></section></div></main>;
}
