...
export function BottomNav({ active }: { active: "home" | "worker" | "venue" | "customer" | "profile" }) {
  return (
    <nav className="bottom-nav" aria-label="Primary">
      ...
      <Link className="nav-customer" href="/tip/T4K8P" aria-label="Customer — tip a worker"><span>♥</span><small>Customer</small></Link>
      <Link className={active === "venue" ? "nav-item active" : "nav-item"} href="/venue"><span>▣</span><small>Venue</small></Link>
      <Link className="nav-item" href="/worker"><span>○</span><small>Profile</small></Link>
    </nav>
  );
}
