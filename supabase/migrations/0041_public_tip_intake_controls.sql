-- SwiftTip MVP v3 — canonical public Tip intake controls.
-- Public transaction intake is OFF by default and cannot become usable merely because
-- pricing/legal records are later activated. A future explicit go-live control must
-- deliberately enable this singleton after provider/legal readiness is approved.

create table if not exists private.runtime_controls (
  singleton boolean primary key default true check (singleton),
  public_tip_intake_enabled boolean not null default false,
  max_tip_intents_per_endpoint_1m integer not null default 120 check (max_tip_intents_per_endpoint_1m between 1 and 10000),
  max_tip_intents_per_endpoint_15m integer not null default 600 check (max_tip_intents_per_endpoint_15m between 1 and 50000),
  updated_at timestamptz not null default now(),
  control_reason text
);

insert into private.runtime_controls(singleton, public_tip_intake_enabled, control_reason)
values (true, false, 'Pre-live default: public Tip intake disabled')
on conflict (singleton) do nothing;

revoke all on private.runtime_controls from public, anon, authenticated;

create index if not exists tips_endpoint_created_idx
  on public.tips(tipping_endpoint_id, created_at desc);

create or replace function private.public_tip_intake_ready(p_at timestamptz default now())
returns boolean
language sql
stable
security definer
...
as $$
  select
    ...
    and exists (
      ...
      where pv.pricing_status = 'active'
        and (pv.effective_from is null or pv.effective_from <= p_at)
...
revoke all on function private.public_tip_intake_ready(timestamptz) from public, anon, authenticated;

create or replace function private.assert_tip_creation_velocity(p_endpoint_id uuid, p_at timestamptz default now())
returns void
language plpgsql
security definer
set search_path = public, private, pg_temp
...
declare
  ...
  v_15m_count bigint;
begin
  select rc.max_tip_intents_per_endpoint_1m, rc.max_tip_intents_per_endpoint_15m
    into strict v_1m_limit, v_15m_limit
  from private.runtime_controls rc
  where rc.singleton;

  select count(*) into v_1m_count
  from public.tips t
  where t.tipping_endpoint_id = p_endpoint_id
    and t.created_at >= p_at - interval '1 minute';

  if v_1m_count >= v_1m_limit then
    raise exception 'Tip intake temporarily limited for this endpoint';
  end if;

  select count(*) into v_15m_count
  from public.tips t
  where t.tipping_endpoint_id = p_endpoint_id
    and t.created_at >= p_at - interval '15 minutes';

  if v_15m_count >= v_15m_limit then
    raise exception 'Tip intake temporarily limited for this endpoint';
  end if;
end;
$$;

revoke all on function private.assert_tip_creation_velocity(uuid,timestamptz) from public, anon, authenticated;
...
as $$
  ...
  where private.public_tip_intake_ready(now())
    and upper(e.short_code) = upper(trim(p_short_code))
...
begin
  if not private.public_tip_intake_ready(now()) then
    raise exception 'Public Tip intake is not enabled';
  end if;
...
begin
  ...
  -- Idempotent replay remains available even if Operations has subsequently closed
  -- new intake, so the original client can recover the resource it already created.
  ...
  -- Serialise new Tip-intent admission for a single Worker endpoint so concurrent
  -- direct-RPC callers cannot race the database-side velocity limit.
  perform pg_advisory_xact_lock(hashtextextended('tip_velocity:' || v_endpoint.id::text, 0));
  perform private.assert_tip_creation_velocity(v_endpoint.id, now());
