...
-- Evidence remains distinct from payment-provider KYC / Settlement destination evidence.

-- Keep the bucket private and retain the existing 10 MB / supported-MIME limits.
...
-- Bind evidence metadata to the exact immutable Storage object that was registered.
...
-- There are no pilot evidence rows at this migration point, so stronger constraints
-- can be established without transforming real identity data.
drop constraint if exists verification_documents_file_size_bytes_check;
alter table private.verification_documents
  add constraint verification_documents_file_size_bytes_check
  check (file_size_bytes > 0 and file_size_bytes <= 10485760);

alter table private.verification_documents
  add constraint verification_documents_mime_type_check
  check (mime_type in ('image/jpeg','image/png','application/pdf'));

alter table private.verification_documents
  add constraint verification_documents_document_type_check
  check (document_type = 'identity_evidence');

alter table private.verification_documents
  add constraint verification_documents_sha256_hash_check
...
create unique index if not exists verification_documents_storage_path_uidx
  on private.verification_documents(storage_path);
create unique index if not exists verification_documents_storage_object_uidx
  on private.verification_documents(storage_object_id);
create index if not exists verification_documents_verification_active_idx
  on private.verification_documents(verification_id, uploaded_at)
  where deleted_at is null;

-- Core evidence metadata is immutable. The only normal post-registration change is
-- the one-way soft-delete marker used after a Worker removes a file while editable.
...
create trigger protect_verification_document_metadata
before update on private.verification_documents
for each row execute function private.protect_verification_document_metadata();

-- Path helpers used by Storage RLS. A valid evidence path has exactly two folders:
-- <worker_id>/<verification_id>/<uuid filename>.
...
begin
  ...
  if v_worker_id is null then return false; end if;
  v_folders := storage.foldername(p_name);
  if coalesce(array_length(v_folders,1),0) <> 2 then return false; end if;
  if v_folders[1] <> v_worker_id::text then return false; end if;

  return exists (
    select 1
    from public.worker_verifications wv
    where wv.worker_id = v_worker_id
      and wv.id::text = v_folders[2]
  );
end;
$$;
...
begin
  ...
  return exists (
    ...
    where wv.worker_id = v_worker_id
      and wv.id::text = v_folders[2]
      and wv.verification_status in ('not_started','additional_info_required')
  );
end;
$$;
...
as $$
  select exists (
    ...
    where vd.storage_path = p_name and vd.deleted_at is null
  );
$$;
...
-- * upload only into the Worker's current editable verification folder;
-- * read only registered active evidence;
-- * Verification/Super Admin direct Storage reads require AAL2;
-- * no UPDATE policy (objects cannot be overwritten);
-- * Worker DELETE is allowed only while that verification is editable.
...
with check (
  ...
  and private.verification_path_is_mutable_for_current_worker(name)
);
...
using (
  bucket_id = 'worker-verification'
  and private.verification_object_is_active_registered(name)
  and (
    private.verification_path_belongs_to_current_worker(name)
    or (
      coalesce(auth.jwt() ->> 'aal','aal1') = 'aal2'
      and private.current_admin_role() in ('verification_admin','super_admin')
    )
  )
);

drop policy if exists worker_verification_evidence_delete on storage.objects;
create policy worker_verification_evidence_delete on storage.objects
...
-- Register only a real Storage object owned by the current Auth user, in the exact
-- current verification folder, while the verification is editable.
...
language plpgsql
security definer
set search_path = public, private, audit, storage
as $$
declare
  v_worker_id uuid;
  ...
  v_active_document_count integer;
begin
  v_worker_id := private.current_worker_id();
  ...
  if not private.verification_path_is_mutable_for_current_worker(p_storage_path) then
    raise exception 'Evidence path is outside the editable Worker verification scope';
  end if;
  if trim(coalesce(p_document_type,'')) <> 'identity_evidence' then
  ...
  if coalesce(lower(trim(p_sha256_hash)),'') !~ '^[0-9a-f]{64}$' then
    raise exception 'A valid SHA-256 evidence fingerprint is required';
  end if;

  select o.id, o.version, o.metadata
  into v_object_id, v_object_version, v_object_metadata
  ...
  if v_object_id is null then
    raise exception 'Verification evidence object does not exist or is not owned by this Worker';
  end if;

  if v_object_metadata ? 'mimetype'
     and coalesce(v_object_metadata ->> 'mimetype','') <> p_mime_type then
    raise exception 'Evidence MIME metadata does not match the registered file';
  end if;
  if v_object_metadata ? 'size'
     and coalesce(v_object_metadata ->> 'size','') ~ '^[0-9]+$'
     and (v_object_metadata ->> 'size')::bigint <> p_file_size_bytes then
    raise exception 'Evidence size metadata does not match the registered file';
  end if;

  select count(*)::integer into v_active_document_count
  from private.verification_documents vd
  where vd.verification_id = p_verification_id and vd.deleted_at is null;
  if v_active_document_count >= 5 then
    raise exception 'A maximum of five active evidence files is allowed per verification';
  end if;

  if exists (
    select 1 from private.verification_documents vd
    where vd.storage_path = p_storage_path or vd.storage_object_id = v_object_id
  ) then
    raise exception 'Verification evidence object is already registered';
  end if;

  insert into private.verification_documents(
    verification_id, storage_path, storage_object_id, storage_object_version,
    document_type, mime_type, file_size_bytes, sha256_hash
  ) values (
    p_verification_id, p_storage_path, v_object_id, v_object_version,
    'identity_evidence', p_mime_type, p_file_size_bytes, lower(trim(p_sha256_hash))
  ...
  values ('worker', auth.uid(), 'verification_document_registered', 'verification_document', v_document_id,
          jsonb_build_object('verification_id',p_verification_id,'document_type','identity_evidence','mime_type',p_mime_type,'file_size_bytes',p_file_size_bytes));
...
-- Worker-owned metadata projection. This deliberately returns no signed URL.
...
begin
  ...
  if not exists (
    ...
    where wv.id = p_verification_id and wv.worker_id = v_worker_id
  ) then
    raise exception 'Verification not found';
  end if;

  return query
  select vd.id, vd.storage_path, vd.document_type, vd.mime_type,
         vd.file_size_bytes, vd.uploaded_at,
         exists (
  ...
  from private.verification_documents vd
  where vd.verification_id = p_verification_id
...
-- Authorise a pre-submission removal and return the exact object path. Physical
-- deletion is performed via the Storage API under its RLS policy.
...
begin
  ...
  select vd.storage_path into strict v_path
  from private.verification_documents vd
  join public.worker_verifications wv on wv.id = vd.verification_id
  where vd.id = p_document_id
    and vd.deleted_at is null
    and wv.worker_id = v_worker_id
    and wv.verification_status in ('not_started','additional_info_required');
...
-- Finalise metadata removal only after the Storage object is genuinely gone.
...
declare
  v_worker_id uuid;
  v_document private.verification_documents;
  v_verification public.worker_verifications;
begin
  v_worker_id := private.current_worker_id();
  if v_worker_id is null then raise exception 'Worker access required'; end if;

  select vd.* into strict v_document
  from private.verification_documents vd
  join public.worker_verifications wv on wv.id = vd.verification_id
  where vd.id = p_document_id and wv.worker_id = v_worker_id
  for update of vd;

  select * into strict v_verification
  from public.worker_verifications
  where id = v_document.verification_id and worker_id = v_worker_id
  for update;

  if v_document.deleted_at is not null then return; end if;
  if v_verification.verification_status not in ('not_started','additional_info_required') then
    raise exception 'Verification evidence cannot be removed in current state';
  end if;
  if exists (
    select 1 from storage.objects o
    where o.bucket_id='worker-verification'
      and o.name=v_document.storage_path
      and o.id=v_document.storage_object_id
      and o.archived_at is null
      and coalesce(o.is_delete_marker,false)=false
  ) then
    raise exception 'Storage evidence must be deleted before metadata removal is finalised';
  end if;
  ...
  values ('worker', auth.uid(), 'verification_document_removed', 'verification_document', v_document.id,
          jsonb_build_object('deleted_at',null), jsonb_build_object('deleted_at','recorded'));
end;
$$;
...
-- Submission requires at least one active registered object and rejects dangling
-- evidence metadata. This means direct Storage deletion cannot create an approvable
-- verification record.
...
begin
  ...
  if not exists (
    ...
    join storage.objects o
      on o.bucket_id='worker-verification'
     and o.name=vd.storage_path
     and o.id=vd.storage_object_id
     and o.archived_at is null
     and coalesce(o.is_delete_marker,false)=false
    ...
    raise exception 'At least one available registered verification document is required';
  end if;
  ...
  if exists (
    ...
    raise exception 'Verification evidence is incomplete; remove or re-upload missing evidence before submission';
  end if;
...
-- Approval itself also rechecks physical evidence availability.
...
begin
  ...
  if p_decision='approve' then
    if not exists (
      ...
      raise exception 'Approval requires available registered identity evidence';
    end if;
    if exists (
      select 1
      from private.verification_documents vd
      ...
      raise exception 'Approval blocked because registered evidence is missing from Storage';
    end if;
  end if;
  ...
  if p_decision='approve' then
    update public.workers set onboarding_status='settlement_pending', updated_at=now()
    ...
    update public.workers set onboarding_status='verification_pending', updated_at=now()
    where id=v_verification.worker_id;
  else
    update public.workers set onboarding_status='blocked', updated_at=now()
    where id=v_verification.worker_id;
  end if;
